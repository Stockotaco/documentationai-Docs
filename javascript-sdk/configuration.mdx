...
<Callout kind="info">
  Tracking the same visitor from a second surface on the page — a Shopify Web Pixel, an embedded widget — and need both to resolve to one person? See the [Identity Storage Contract](/javascript-sdk/identity-storage-contract).
</Callout>
...
<ParamField body="capture_heatmaps" param-type="boolean|object" required="false" show-location="false" deprecated="false">
  Capture click positions and scroll depth for [heatmaps](/heatmaps-api/overview). Default: `false` (off). Pass `true` to enable with defaults, or an object for control:
  ...
  This is **not** the same thing as `autocapture`. Autocapture records *which element* was clicked — its text, `href`, classes. This records *where on the page*, plus a structural description of the clicked element so heatmaps stay correct on pages that change after load: tag, a stable id, up to two classes, its position among same-tag siblings, an identifier-style `data-*` attribute, and a link's path. It never reads element text or query strings. Enable either, both, or neither.
...
<ParamField body="capture_experiments" param-type="boolean|object" required="false" show-location="false" deprecated="false">
  Detect that a third-party split-testing tool has bucketed this visitor and attach the assignment to the session. Default: `true` (on). Supported platforms: **Convert.com** and **PostHog experiments**. Pass `false` to disable, or an object for control:
  ...
  Nothing needs tagging on the page. Every event after detection carries two super properties — `$experiments` (an array of arms) and `$ab_platform` (`convert`, `posthog`, or `convert+posthog`) — and each assignment produces one `$experiment_viewed` event per session, with `experiment_id`, `variation_id` and `variation_name` where the tool exposes them.

  An arm reads `"<experimentId>:<variationId>"` for Convert.com and `"posthog:<flagKey>:<variant>"` for PostHog. Only PostHog arms carry a platform prefix, so filters written against Convert assignments keep working unchanged.

  Convert.com assignments are read from its own cookie, which holds the authoritative experiment-to-variation mapping and is already populated for visitors who arrived through a split-URL redirect; its `dataLayer` announcement is observed as well, to catch bucketing that happens later in a single-page app and to pick up variation names.

  PostHog assignments are read from posthog.js's own persisted state (`ph_<projectToken>_posthog__flags` for the flags, `ph_<projectToken>_posthog` for the visitor id and any local overrides), refreshed through the public `posthog.onFeatureFlags` subscription when flags load or reload. A redirect-style split test needs nothing extra: the flag is already stored by the time the variant page loads, so the assignment is attached before that page's first pageview. A flag counts as an experiment when PostHog reports it as experiment-linked, and otherwise when it is multivariate: **boolean feature flags are rollouts, not tests, and are excluded**, so a kill switch does not become an arm on every event. Flags you have overridden locally (`overrideFeatureFlags()`, or the PostHog toolbar) are skipped as QA. The SDK never calls `getFeatureFlag()` or `isFeatureEnabled()` — those would write `$feature_flag_called` events into your PostHog project.

  Sessions the SDK identifies as bots record no assignment — a crawler executing a testing tool would otherwise count as an exposure in whichever arm it landed in.
</ParamField>

<Callout kind="info">
  On a page with no testing tool this costs one cookie read and one localStorage scan: no properties are published, and your `dataLayer` is not touched or created. Observers are only attached once a testing tool has actually been detected.
</Callout>
...
<ParamField body="experiment_conversions" param-type="boolean|object" required="false" show-location="false" deprecated="false">
