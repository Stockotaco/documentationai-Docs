...
<Steps>
  <Step title="Paste the loader snippet" icon="download" titleType="p">
    Drop this into `<head>`, as high as possible. It installs the error listeners, stubs `window.mythic`, asynchronously loads the CDN loader (~1.4KB) and the SDK core, and calls `init` — one tag, nothing else to add.
