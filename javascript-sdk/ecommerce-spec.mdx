...
One sale reaches Mythic from both producers and purchase dedupe keeps one copy — usually the browser's, because it arrives first. So `customer_lifetime_value` is the one to check for before you depend on it. `orders_count` and `total_spent` are also written to the **person profile**, which is independent of dedupe — filter there when you need them to always be present.
