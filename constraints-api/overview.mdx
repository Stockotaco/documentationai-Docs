...
Every endpoint here is read-only — there is nothing to create.
...
```json
{
  ...
  "events": [
    ...
    { "event_name": "ghl_rebill", "total_events": 3000, "session_coverage": 0,
      "person_coverage": 0, "credited_via": null, "suggested_role": null,
...
`suggested_role` repeats the `suggested` pairing on the events themselves: `conversion` and `lead` on the two suggested events, `null` on the rest. It is not a per-event classification, so a `null` event can still be passed as `lead_event` or `conversion_event`.
...
"tracking": {
  "match_rate": 0.91,
  "unexpanded_macro": {
    "sessions_all_sources": 189,
    "sessions_in_unmatched": 4,
    "sample": "{campaignname}",
...
`sessions_all_sources` counts every session with the broken tag, whatever brought it. `sessions_in_unmatched` is the subset that arrived on a Meta ad click and sits in `unmatched_sessions`, so it is the share of the tracking hole the macro explains; it can be far smaller than the total. The older names `sessions` and `meta_click_sessions` carry the same two counts and are deprecated.
