...
- **Geo stream** (preferred) — `{ "source": "geo_stream", "table": "your_geo_insights_table", "region_col": "region" }`. Uses a region-broken Meta insights stream (a custom Airbyte report with a `region`/`dma` breakdown), so test-region spend is read directly from Meta. The default Meta insights table has no region column, so a geo stream always needs its own `table`.
