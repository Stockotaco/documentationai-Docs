...
| `cooldown_days` | *(optional)* Extend the measured window past `test_window.to` to capture lagged conversions. |
| `exclude_dates` | *(optional)* Dates to drop from the analysis (use for reported `outlier_days`). |
| `alert_webhook_url` | *(optional, https)* Daily drift alerts while the test runs. |
