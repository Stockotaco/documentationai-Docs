...
<Callout kind="warn">
  **`spend.source: "geo_stream"` can never produce a coefficient — this is a Meta platform rule, not a data gap.**
  ...
  Meta classifies `region` as a **Type 1 breakdown**, and [its Insights API docs](https://developers.facebook.com/docs/marketing-api/insights/breakdowns/) state it "will not return unsupported off-Meta metrics (for example, an actions metric with Type 1 breakdowns)." On-Meta metrics — impressions, link clicks, post engagement — still come back; **offsite purchase conversions and their values do not**, for every advertiser. No Airbyte field selection or connector change alters this.
