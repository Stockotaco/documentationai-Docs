...
<Callout kind="warning">
  **This is not the endpoint an AI assistant connects to.** It answers one JSON-RPC object per `POST`, with no `Mcp-Session-Id` and no event stream, so an MCP client that opens a connection with `GET` receives a `404`. Point Claude, Cursor, TypingMind and the rest at the [Settings Server](/mcp/settings-server), which speaks Streamable HTTP and takes the same `mcp_` key.
</Callout>
