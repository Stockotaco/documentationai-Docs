...
Returns every tool the key can use — built-in tools allowed by scope, plus upstream tools allowed by scope, agency access, and location access. Each entry has `name`, `description`, `inputSchema`, and `annotations`. Tools come back sorted by name, so the list is stable across calls and safe to cache. The result also carries `ttlMs` (300000) and `cacheScope` (`"private"`) as caching hints.
