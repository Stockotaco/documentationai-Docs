...
Every tool is **listed** whatever the key's scopes. Calling one the scopes do not
cover answers a tool error with `code: "insufficient_scope"`, the `required`
scope and the scopes `held`, so the assistant can tell you exactly what to add
instead of reporting an unknown tool.
