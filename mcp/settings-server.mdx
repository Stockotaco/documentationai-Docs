---
title: Connect an AI client (Settings Server)
description: >-
  Connect Claude, Cursor, TypingMind or any MCP client to the hosted Settings
  Server — the full agency product surface, 204 tools across data, sessions,
  replays, analysis, settings, links, Airbyte, segments, insights, dashboards,
  canvases, destinations, contracts, tenant config, and agency-grain rollups —
  with a select-a-client session flow.
---

## Overview

The Settings Server is a hosted MCP server exposing the **full agency product
surface** — 204 tools across 25 scope families — from any MCP-compatible AI
client. It is the URL your AI client connects to. It is separate from the
...
The transport is **Streamable HTTP** (called "HTTP", "streamable-http" or "remote MCP server" depending on the client). Give the client the URL above and one header.

There is no OAuth flow — configure the key as a static header. The server says so at both places a client looks: a request without a usable key answers `401` with an [RFC 6750 challenge](/mcp/authentication#what-a-401-tells-you), and every OAuth/OIDC discovery path answers `404 {"error": "oauth_not_supported"}` naming the credential to send instead. A client that can *only* authenticate by discovering an authorization server cannot connect.
...
curl -X POST https://mythic-mcp-server-production.up.railway.app/mcp \
...
|----------|:---:|----------|
| **Client-scoped** | set | Fixed to one client. Tools operate on it; navigation tools are hidden. A `location_id`/`tenant_key` naming a different client is a tool error, not an answer for the bound client. |
| **Agency-wide** | `null` | Not bound to a client. Gets `list_clients` / `select_client`; a client must be selected before client-scoped tools work — or connect to [`/mcp/c/<location_id>`](#pinning-a-client-in-the-url). |
...
client for that one call — so passing it always is the portable choice. Or pin
the client [in the URL](#pinning-a-client-in-the-url) and never pass it.
...
proxies of the [Agency API](/agency-api/overview) (`/client/v1/agency`), so
...
The **alert tools** (`list_alerts`, `get_alert`, `resolve_alert`, and the rule
and channel tools) are also agency-wide only and take `alerts:read` /
`alerts:write`. They proxy the [Alerts API](/agency-api/alerts).

Scope families gate the tools — the full table, with what each one
grants, is on the [Authentication](/mcp/authentication#families) page:
...
`settings`, `links`, `tags`, `snippets`, `transformers`, `airbyte`, `alerts`, `agency`.

**[See the Tool Reference](/mcp/tools)** for every tool by name, the scope it
needs, what it does, and the API route it proxies. That page is generated from
the server's own registry, so it is never out of step with what the server serves.
The same scope authorizes the proxied route when the key is
[used directly over HTTP](/mcp/authentication#using-an-mcp_-key-over-http).
...
Three prompts encode the tool sequences agencies run most, so the assistant does
not rediscover them each session:
...
Arguments are validated strictly. Every tool schema sets
`additionalProperties: false`, and an argument the tool does not declare fails
the call with `Input validation error: … Unrecognized key: "<name>"` instead of
being dropped.
