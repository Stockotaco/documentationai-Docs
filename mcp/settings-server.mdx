...
The transport is **Streamable HTTP** (called "HTTP", "streamable-http" or "remote MCP server" depending on the client). Give the client the URL above and one header. There is no OAuth flow: the server publishes no `.well-known` metadata and a client that insists on discovering one cannot connect — configure the key as a static header.
