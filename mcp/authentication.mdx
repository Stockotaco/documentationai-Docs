...
An `mcp_` key carries a `scopes` array that gates which tools it can call (every tool is listed; a call outside the scopes answers `insufficient_scope` naming the scope to add) — and, when the key is sent as a Bearer to the HTTP API, which routes it may call. Scopes use `<family>:<action>` form, with `*` wildcards:
