...
description: >-
  Authenticate Data API requests with an agency key, a location secret key or
  a scoped key, and bind a location to scope the data you receive.
---
...
| Scoped key | `mcp_` | Only the routes its [scopes](/mcp/authentication#using-an-mcp_-key-over-http) grant — `people:read`, `events:read`, `sessions:read`, `replays:read`, `exceptions:read`, `heatmaps:read`, `bigquery_export:read` here |
