...
Every endpoint is a `GET` except `POST /query`, whose body is a query spec. The API deliberately sends no CORS headers: keys are server-side credentials and must never be embedded in a browser.
