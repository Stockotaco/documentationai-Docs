...
`mythic_error_code` comes from a closed set you can branch on — `state_invalid`, `missing_secret_id`, `missing_account_id`, `not_configured`, `not_provisioned`, `oauth_secret_expired`, `oauth_credentials_missing`, `account_not_accessible`, `connection_failed`. The three OAuth-specific ones tell a human what to do: `oauth_secret_expired` means start the connection again, `account_not_accessible` means consent worked but the account id sent at initiation is not one that user can reach, and `connection_failed` is the catch-all for everything else. Exception text never travels in the URL; it stays in server logs. Errors from `/oauth/initiate` carry the same idea as a `code` field in the JSON body (`return_url_not_allowed`, `missing_account_id`, and so on) — switch on `code`, not on the prose in `error`.

A failed callback also writes a sync-event row with `error_origin: oauth_callback` and the underlying Airbyte message in `error_message`, readable at `GET /sync-events?client_id=…`. That is the only trail a failed consent leaves — no connection row is created — so surface it when a user comes back with `mythic_oauth=error`.
