...
Delivery is best-effort and there is no redelivery queue: the event is already durable, so reconcile a gap from `GET /sync-events` rather than waiting for a retry. An endpoint that fails 20 times in a row is parked (`active: false`) so a dead URL stops costing a request on every sync — POST the same URL again to resume it. Posting a URL that already exists updates it rather than adding a second subscription, which would deliver everything twice. Remove a subscription entirely with `DELETE /webhooks/{id}`.
