...
| **Filter values** | Which devices/sources reached this page? | `GET /client/v1/data/heatmap/facets` |
...
## Element-anchored clicks

Coordinates break on pages that change after they load. A 2-step order form that inserts its payment section after the first click moves everything below it down by that section's height, for exactly the visitors who used it. Their clicks on the reviews and footer land 1,500px below those elements on a background captured at load, and clicks on the payment fields land on nothing.

Since SDK 3.26, every heatmap click also records the element it hit: a short chain from the target up (tag, stable id, up to two classes, position among same-tag siblings, an identifier-style `data-*` attribute, a link's path, never element text) and where inside that element's box the click landed. `GET /client/v1/data/heatmap?mode=element` returns clicks grouped that way:
...
```json
{
  "data": {
    ...
    "total_clicks": 5148,
    "anchored_clicks": 812,
    "anchored_share": 0.158,
    "elements": [
      {
        "selector": "section.reviews:nth-of-type(2) > div.review-card:nth-of-type(2)",
        "chain": [{ "t": "div", "n": 2, "k": ["review-card"] }, { "t": "section", "n": 2, "k": ["reviews"] }],
        "clicks": 41,
        "visitors": 30,
        "offsets": [{ "ox": 0.52, "oy": 0.4, "clicks": 12 }]
      }
    ]
  }
}
...
To draw it, match each `chain` against the rebuilt background's DOM (target level first, every ancestor must agree on tag; an `i` or `d` identifies the element on its own, and classes, `n` and `h` score the rest), then place each offset at `rect.left + ox * rect.width`, `rect.top + oy * rect.height`. The element can sit anywhere in the background, so inserted content no longer moves the dot. A chain with no match is an element the background doesn't have: count it and don't draw it. Drawing it at its coordinates brings the original bug back. `selector` is descriptive for people and agents, not a guaranteed-unique selector.

Two things break matching silently. Measure boxes with the rebuilt page scrolled to the top, instantly: rrweb restores the visitor's scroll offset, and on a page with `scroll-behavior: smooth` both that restore and your `scrollTo(0, 0)` animate, so a fixed header measured mid-scroll lands hundreds of pixels low. And prefer a visible element among equally good matches: page builders clone elements (Elementor's sticky header leaves a hidden spacer with identical classes).

`anchored_share` says how much of the window carries elements. Points captured before SDK 3.26 have coordinates only and stay readable in the default `mode=coordinate`.

A load-time background still can't show content that only exists later. `GET /heatmap/snapshot?state=final&min_page_height=<px>` returns a whole recorded visit of the page whose page ended at least that tall. Replay it to its last event and the background shows the page as that visitor last saw it (the payment step open). Use `page_heights` (below) to pick the height.
...
```json
{
  "data": {
    ...
    "viewport":    [{ "value": "mobile", "sessions": 440, "min": 0, "max": 767 },
                    { "value": "desktop", "sessions": 61, "min": 1280, "max": 20000 }]
  }
}
```
...
By default the click map mixes every screen size. Don't draw that mix on a page. A 1280px layout and a 390px layout are different pages that share a URL. `x_rel` only lines up across widths for fully fluid layouts, and `y_px` changes completely when a layout reflows. On a real paid landing page, 89% of clicks came from phones and 7% from desktops, spread over 104 distinct widths.

Read one layout bucket at a time. Take the buckets from `/heatmap/facets` (`viewport`, each with its `min`/`max`), default to the one with the most sessions, and pass the same range to the click, scroll **and** snapshot calls, so the background is captured at that layout too:

```bash
# desktop only
curl "https://mythic-analytics.gulp.workers.dev/client/v1/data/heatmap?url=acme.com/pricing&viewport_min=1280" \
  -H "Authorization: Bearer sk_live_..."
```

Narrower window → truer positions, less data. Wider → more data, blurrier positions. There is no setting that gives you both. A bucket is the practical default. Inside the desktop bucket, a centred fixed-width layout still shifts horizontally between a 1280px and a 1920px screen, so narrow `viewport_min`/`viewport_max` around the background's `width` when you need pixel precision there.
...
Candidates are recorded page loads of that URL, newest first, inside the viewport range you pass. Mid-funnel pages work without a `session_id`. A snapshot from another breakpoint is never substituted. With no match you get `404`, and you should draw on a blank stage at the bucket's typical width rather than a background from another layout.
...
### Six traps, all of which look like working code
...
| Trusting a load-time background on a page that grows | Clicks in blank space below the footer, and clicks lower down drawn on the wrong content | The background is the page as it loaded. A 2-step order form or an accordion inserts content later, and everything below the insertion shifts for the visitors who triggered it. `page_heights` on the click and scroll responses gives each session's end-of-visit page height. Measure where the rebuilt background's content ends, and warn when a real share of sessions ended well past it |
| Stretching the iframe without freezing `vh` | A full-screen hero thousands of px tall, everything below it shifted down | Once the iframe is document-tall, `100vh` in the page's CSS means the document height. After the snapshot rebuilds (`fullsnapshot-rebuilded`), rewrite `vh`/`vmin`/`vmax` in the iframe's readable stylesheets to px at `snapshot.height` |
...
// One layout at a time: the busiest bucket from /heatmap/facets.
const facets = await fetch(`${base}/client/v1/data/heatmap/facets?url=${page}`, { headers }).then((r) => r.json())
const bucket = [...facets.data.viewport].sort((a, b) => b.sessions - a.sessions)[0]
const vw = `viewport_min=${bucket.min}&viewport_max=${bucket.max}`

const [snap, heat, scroll] = await Promise.all([
  fetch(`${base}/client/v1/data/heatmap/snapshot?url=${page}&${vw}`, { headers }).then((r) => r.json()),
  fetch(`${base}/client/v1/data/heatmap?url=${page}&${vw}`, { headers }).then((r) => r.json()),
  fetch(`${base}/client/v1/data/heatmap/scroll?url=${page}&${vw}`, { headers }).then((r) => r.json()),
])
...
- **Not text-aware.** `mode=element` knows an element's structure, not its words. For "which button got clicked, by label", query `$autocapture` events through the [Data API](/data-api/overview), which records element text.
...
All five endpoints are reads and accept either key; toggling capture is a Settings write and needs an agency key.
