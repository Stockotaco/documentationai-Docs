...
import TierGated from "/snippets/tier-gated.mdx"
...
Reads accept either key. **Writes require an agency key** — same rule as Settings, Destinations, and constraint benchmarks.
...
- **Default window is the start of yesterday (UTC) to now**, 24 to 48 hours depending on when you ask. The response's `window` spells it out as timestamps. Set `date_from` / `date_to` (ISO 8601) for anything else.
...
<Callout kind="warning">
  The report scans only the contracted events. A `truncated: true` flag on the response means their property statistics still hit the row limit. Narrow the window or pass `?event=` before trusting `absent` counts — a truncated read under-counts keys and can manufacture "missing" fields that are really a row limit.
</Callout>
