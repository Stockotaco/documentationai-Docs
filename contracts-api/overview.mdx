...
Read the current configuration back with `GET /client/v1/settings/contract-alerts?location_id=$LOC`.
...
### Triggering the sweep on demand

The daily sweep runs at ~08:00 UTC. To run it immediately — for testing a webhook endpoint or inspecting what it would send — call:

```bash
curl -X POST "https://mythic-analytics.gulp.workers.dev/admin/contracts/alert-sweep" \
  -H "Authorization: Bearer $ADMIN_KEY"
```

Pass `?dry_run=1` to compute everything and POST nothing. The response includes per-location `would_send`, `reason` (fingerprint unchanged, first-sweep baseline, truncated scan, or no contracts), and `payload` — the exact JSON each receiver would get. This endpoint requires an admin key, not an agency key.
