...
The sweep runs at ~08:00 UTC over the previous whole UTC day — closed, so late-arriving data has landed. Each POST carries `X-Mythic-Event: contracts.alert`; with `secret` set, it also carries `X-Mythic-Signature: sha256=<HMAC-SHA256 of the raw body>`.

**One webhook per location, not per contract.** The most common thing worth alerting about is a tracking outage, and that breaks every contract at once. Per-contract delivery would send thirty alerts for one cause, so the outage case is exactly one payload with `type: "contracts.tracking_outage"`. Schema violations on one or more events produce `type: "contracts.contract_violations"` — also one payload per location.
