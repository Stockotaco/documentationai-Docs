...
| `GET /agency/contracts/summary` | location | Declared event contracts, whether the daily alert webhook is wired, whether an [alert rule](/agency-api/alerts) routes contract alerts, and when the last webhook alert fired |
| `GET /agency/usage` | day × location × surface × route (or day × surface × route with `group_by=route`) | The agency's own API + MCP request telemetry — the same data Mythic meters on |
| `GET /agency/quota` | quota in force | The plan, the features it includes, and month-to-date ingested events against the quota |

Alerts across every client (the feed, routing rules and Slack/webhook channels)
live on their own surface: see [Alerts](/agency-api/alerts).
...
| Status | Code | Meaning |
| --- | --- | --- |
| `400` | `invalid_group_by` | `GET /agency/usage`: `group_by` is not `location` or `route` |
...
It returns at most 1000 rows and sets `truncated: true` when the cap cut the
result. A busy agency passes 1000 location-grain rows within a week, so use
`group_by=route` to fit a month.
