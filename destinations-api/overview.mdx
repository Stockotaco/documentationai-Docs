...
| **Convert.com experiments** | `convert_experiments` | Credits an off-browser conversion to the A/B variation the visitor was in | None for delivery (see [Convert.com experiments](#convertcom-experiments)) |
...
The endpoint returns `200` for everything, including payloads it discards — so a success in the delivery log means "accepted", never "counted". Use [verification](#verifying-a-convert-destination) to find out what actually landed.
...
<Callout kind="info">
  ...
  `maryjane`). Google additionally asks for honorifics and generational or
  professional suffixes to be absent, so `Mrs. Mary` is hashed as `mary` and
  `Smith Jr.` as `smith`. A name that is only a title contributes nothing
  rather than hashing the title itself.
</Callout>
...
City and state keep their internal spaces (`New York` is sent as `new york`)
and their accents, which is Google's documented rule and deliberately unlike
the Meta path, where the same city is hashed as `newyork`.
...
Three reads, for three different questions, all `meta_capi` only.

- `GET /{id}/quality` is the **live** read: Meta's current scoring of the dataset, per event name, with per-parameter coverage and the findings behind it. It uses the destination's own stored token, so no second credential is involved.
- `GET /{id}/quality/trend` is the **daily series**: when the score moved. Meta returns only the current score and keeps no history of its own, so this comes from Mythic's own daily snapshot. A day with no row means nothing was captured that day — no value is carried forward, because a flat line through an outage reads as stability.
- `GET /{id}/quality/match-keys` is the **diagnosis**: the same days split per match key (`em`, `ph`, `fbp`, `fbc`, `external_id`, `ip_address`, `ct`, `st`, `zip`, `country`). Match quality is a composite, so a fall in it names no fix; the parameter whose coverage moved with it does.

Across all three, a `null` metric means Meta reported nothing — not zero. A key listed with `null` coverage was not scored, which is a different finding from a key that is never sent.

Agency-wide, `GET /client/v1/agency/destinations/quality` ranks the roster by current state and `GET /client/v1/agency/destinations/quality/trend` gives the same roster day by day.
