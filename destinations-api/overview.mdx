...
**There is no delivery credential.** Convert's tracking endpoint is unauthenticated and returns `200` for everything, including payloads it discards — so a success in the delivery log means "accepted", never "counted". Use [verification](#verifying-a-convert-destination) to find out what actually landed.
...
<Callout kind="note">
  Convert's report lags roughly 60–90 seconds behind a delivery, so a verification run immediately after a test fire will under-count.
</Callout>
