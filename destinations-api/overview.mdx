...
Both URLs share the same **15-minute** state window. The `start_url` is replayable — it can be opened multiple times within that window because it only redirects to Google and does not consume the state token. The consent itself is **single-use**: once the callback fires, the state token is consumed and a new `initiate` call is required to start over. On completion the browser is sent to `return_url` with `mythic_oauth=success` (or `mythic_oauth=error&mythic_error=<code>`) appended; omit `return_url` and the callback renders a terminal page instead.
