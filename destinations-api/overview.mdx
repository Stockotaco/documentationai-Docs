...
Requires `account_id`, `project_id` and `goal_id` — all Convert dashboard ids, all numeric. Optional `sdk_key`, `send_revenue` and `timeout_ms`. Which events reach the goal is decided by `trigger_rules`; there is no `event_mapping`.
...
```json
{
  ...
  "trigger_rules": { "operator": "AND", "rules": [
    { "type": "mythic_event", "event_name": "transaction_completed" }
  ]},
  "action_config": {
...
**Revenue.** Conversions and revenue are separate counters in Convert with separate de-duplication rules, so `send_revenue` is additive — the goal hit is still sent. It is only meaningful when `goal_id` names a Convert goal of type **Revenue**; pointed at any other goal type, the effect is undefined. The amount comes from the conversion event's value, and a missing, zero or unparseable value sends no revenue rather than a number we cannot stand behind.

**There is no delivery credential** — Convert's account/project tracking endpoint takes no authentication at all. That also means anyone who knows your account and project ids can write conversions into your project, so set `sdk_key` if you have one and conversions go to Convert's authenticated endpoint instead.

Either way the endpoint returns `200` for everything, including payloads it discards — so a success in the delivery log means "accepted", never "counted". Use [verification](#verifying-a-convert-destination) to find out what actually landed.
...
| `shortfall` | Convert counted fewer than the distinct visitors delivered. Deliveries are being discarded. |
...
| `report_unavailable` | Convert's report could not be read. No verdict rather than a guess. |
