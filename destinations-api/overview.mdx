...
You do not have to mint a refresh token by hand. `POST /destinations/oauth/initiate` returns a `consent_url` and a shorter `start_url`; send the browser to either one, the user picks a Google account, and the callback stores a credential for you. It is the same credential shape either way — the consent flow is a better way to obtain it, not a different one.

The `consent_url` is Google's full consent-screen URL (~430 characters). The `start_url` is a compact redirect (~127 characters) that is safe to paste into emails, terminals, and chat — the raw `consent_url` can truncate in those contexts and produce a 404 at Google. Both resolve to the same consent screen; the `start_url` does not consume the state token, so clicking it twice is harmless.
