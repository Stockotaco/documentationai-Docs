...
1. `clientId` — from `$ga_client_id`, which the JavaScript SDK reads from Google
   Analytics' own `_ga` cookie. This is the **only** key that joins the event to
   a session GA already started.
