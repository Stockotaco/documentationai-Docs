...
description: >-
  ...
  the Meta Conversions API, Google Ads, and Google Analytics 4, with
  per-destination credentials, delivery logs, and health monitoring.
---
...
You do not have to mint a refresh token by hand. `POST /destinations/oauth/initiate` returns a `consent_url` and a shorter `start_url`; send the browser to either, the user picks a Google account, and the callback stores a credential for you. It is the same credential shape either way — the consent flow is a better way to obtain it, not a different one. The `start_url` is a ~127-character shareable link that redirects to the full `consent_url` — use it when the raw URL (430+ characters) would truncate in chat, email, or a terminal. It is unauthenticated, so you can send it to someone who has no API key.
