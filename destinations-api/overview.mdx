...
You do not have to mint a refresh token by hand. `POST /destinations/oauth/initiate` returns a `consent_url` plus a `start_url`; send the browser to either one, the user picks a Google account, and the callback stores a credential for you. `start_url` is a short, shareable redirect link that is safe to email to a client who has no API key: it points to `GET /destinations/oauth/start?t=...`, an unauthenticated browser endpoint that redirects to Google's consent screen without exposing the original consent URL. It is the same credential shape either way — the consent flow is a better way to obtain it, not a different one.
...
The `consent_url` and `start_url` are valid for **15 minutes**. The underlying state token is **single-use**: the callback consumes it on completion, but `start_url` itself can be opened multiple times within that window because `GET /destinations/oauth/start` does not consume the token. On completion the browser is sent to `return_url` with `mythic_oauth=success` (or `mythic_oauth=error&mythic_error=<code>`) appended; omit `return_url` and the callback renders a terminal page instead.
