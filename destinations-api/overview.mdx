...
Four destination types are supported. The `action_config` you send is validated against the `action_type`.
...
<Callout kind="info">
  `tiktok_events` is recognized by the schema but has no executor yet, so creating it returns `unsupported_destination_type`. Only `webhook`, `meta_capi`, `google_ads`, and `google_analytics` are live.
</Callout>
...
Requires a fetchable `url`. Optional `method`, `headers`, `body_template`, `timeout_ms`, and `retry_count`. Because webhook credentials live inline in `action_config`, header values and `body_template` are redacted to `***` when read with a secret (viewer) key. HIPAA-enabled locations reject `http://` URLs at write time. They also reject `google_analytics` destinations entirely — GA4 user-data fields cannot be guaranteed HIPAA-compliant.
...
- **Ingestion is asynchronous.** In practice it takes around **30 minutes** to go
  ...
  nothing, correctly.
- **DebugView and Realtime are not valid tests.** Set `debug_mode: true` on the destination config to route events into GA4 DebugView — the only practical way to verify a tagless setup end to end, since there is no browser console. Realtime is still not a valid test: Data Manager is a backend data source rather than a tag hit, so it does not flow through the realtime pipeline. Look in standard reports, or the BigQuery export.
...
The initiate response also includes a `start_url` — a shareable link that redirects to the consent screen without an API key. Give it to the person who needs to consent; it is replayable within the 15-minute window (the token is consumed only on callback, not on start).
...
<ResponseField name="warnings" field-type="array" required="false" deprecated="false">
  Non-blocking validation warnings, present on create responses when the destination is accepted but has potential issues — e.g. a GA4 `event_mapping` that maps to a GA4 enhanced-measurement event name. Each entry has a `field` and `message`.
</ResponseField>
