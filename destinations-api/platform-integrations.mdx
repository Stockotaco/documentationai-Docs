...
### Generate the token with Dataset Quality enabled

In Events Manager: **Settings → Conversions API → Set up direct integration → "Set up with Dataset Quality API"** (the recommended option), then copy the token it shows.

That one token does both jobs. It sends conversions, and it lets
[`GET /destinations/{id}/quality`](/destinations-api/overview) read Meta's own scoring of the data you send it — event match quality, per-parameter coverage, and the dedupe-key coverage that proves the pixel and CAPI halves are collapsing into one conversion rather than double counting. Both permissions are scoped to that single dataset.

A token generated with **"Set up without Dataset Quality API"** delivers conversions normally and returns `409 insufficient_permission` on the quality endpoint. Regenerate it with the recommended option and store it again; delivery is unaffected either way.
...
Each event needs a `conversion_action` (server half) — the **bare conversion action ID**, which is sent as the Data Manager `productDestinationId`. A Google Ads resource name (`customers/1234567890/conversionActions/987654321`) is accepted and its trailing ID used, so a value copied from the Google Ads API still works. Anything else is rejected as a permanent configuration error rather than failing later as an opaque Google error. A `conversion_label` additionally turns on the managed gtag browser tag for that event, with `transaction_id` defaulted to the event uuid so browser and server sends dedupe. Events without a label are server-only.
