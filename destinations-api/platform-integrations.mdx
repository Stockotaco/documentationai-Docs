...
<Callout kind="info">
  ...
  own `capture()` names. By default `$pageview` fires the **browser pixel
  only**: server-side pageviews multiply Conversions API calls without adding
  conversion data. Map `$pageview` explicitly to send it both ways.
</Callout>
